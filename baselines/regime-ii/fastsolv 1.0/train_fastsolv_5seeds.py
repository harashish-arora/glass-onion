...
def save_seed_ensemble(models, seed, model_dir):
    seed_dir = Path(model_dir) / f"seed_{seed}"
    seed_dir.mkdir(parents=True, exist_ok=True)

    saved_paths = []
    for rep_idx, model in enumerate(models, start=1):
...
def train_one_seed(seed, df_train, df_test, test_var):
    ...
    # 4. Train ensemble with ShuffleSplit (random replicates)
    ...
    for rep, (tr_idx, val_idx) in enumerate(splitter.split(indices)):
        ...
        trainer = Trainer(
            max_epochs=100, accelerator="cpu", precision=32, 
...
def main():
    ...
    for seed in SEEDS:
        ...
        rmse, r2, models = train_one_seed(seed, df_train, df_test, test_var)
        elapsed = time.time() - start_time

        saved_paths = save_seed_ensemble(models, seed, MODEL_DIR)

        results.append((rmse, r2))
        times.append(elapsed)
        print(f"Seed {seed} | RMSE: {rmse:.4f} | R²: {r2:.4f} | Time: {elapsed:.1f}s")
        print(f"Saved {len(saved_paths)} replicate models under {Path(MODEL_DIR) / f'seed_{seed}'}")
    ...
    artifact_csv = Path(MODEL_DIR) / "saved_model_artifacts.csv"
    artifact_df.to_csv(artifact_csv, index=False)

    summary_json = Path(MODEL_DIR) / "training_summary.json"
    summary_payload = {
        "seeds": SEEDS,
    ...
    print(f"Saved model artifact index: {artifact_csv}")
